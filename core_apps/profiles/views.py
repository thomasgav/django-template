from django_template.settings.local import DEFAULT_FROM_EMAIL
...
class FollowAPIView(APIView):
    def post(self, request, user_id, format=None):
        try:
            ...
            #following itsself error
            if profile == follower:
                raise CantFollowYourself()

            #following already followed error
...
class UnfollowAPIView(APIView):
    def post(self, request, user_id, *args, **kwargs):
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                "status_code": status.HTTP_400_BAD_REQUEST,
                "message": f"You can't unfollow {profile.user.first_name} {profile.user.last_name}, since you were not currently following them.",
            }
